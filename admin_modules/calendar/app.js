...
function pendingActionButtons(kind, pendingId) {
    return `
        <button class="secondary-btn" style="padding:6px 10px; font-size:0.75rem; width:auto; border-radius:6px; background:rgba(16, 185, 129, 0.15); color:#10b981; margin-bottom:0; border: 1px solid rgba(16, 185, 129, 0.2);" onclick="approvePending(${pendingId}, '${kind}')">
            Approve
        </button>
        <button class="secondary-btn" style="padding:6px 10px; font-size:0.75rem; width:auto; border-radius:6px; background:rgba(239, 68, 68, 0.15); color:#ef4444; margin-bottom:0; border: 1px solid rgba(239, 68, 68, 0.2);" onclick="rejectPending(${pendingId}, '${kind}')">
            Reject
        </button>
    `;
}
...
async function approvePending(id, kind) {
    ...
    const api = kind === 'meeting' ? '/api/meetings/approve' : '/api/calendar/approve';
...
async function rejectPending(id, kind) {
    const label = kind === 'meeting' ? 'scheduled meeting' : 'calendar event';
    if (!confirm(`Reject and discard this ${label}?`)) return;
    const api = kind === 'meeting' ? '/api/meetings/reject' : '/api/calendar/reject';
    const res = await fetch(api, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id })
...
async function renderEventDetailContent() {
    ...
    if (actions) {
        ...
        actions.innerHTML = docsBtn + (ev.pendingId ? pendingActionButtons('event', ev.pendingId) : '');
    }
}
...
async function renderMeetingDetailContent() {
    ...
    if (actions) {
        ...
        actions.innerHTML = docsBtn + (m.pendingId ? pendingActionButtons('meeting', m.pendingId) : '');
    }
}
...
window.closeDocumentsModal = async function () {
