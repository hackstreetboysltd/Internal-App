 ...
 */
const PUT_BODY_IS_PARTIAL = new Set(["messages", "channels", "goals"]);
...
function isAuthError(err) {
  return err instanceof DataApiError && (err.status === 401 || err.status === 403);
}
...
export function watchCollection(collectionName, onData, options = {}) {
  ...
  fetchCollection(collectionName, { ...options, cacheFirst: false })
    ...
    .catch((err) => {
      if (stopped) return;
      // Signed-out / login-redirect: not a collection failure.
...
export async function putCollection(collectionName, data, options = {}) {
  ...
  // Always seed the local cache with the saved payload.
...
/**
 * Soft-delete one row. Used for goal deletes so a member save does not rewrite
 * the rest of the collection (the path that timed out at 15s).
 *
 * @param {string} collectionName
 * @param {string} id
 * @param {{ admin?: boolean }} [options]
 */
export async function deleteCollectionItem(collectionName, id, options = {}) {
  let res;
  try {
    res = await apiFetch(
      apiFetchPath(
        `/api/data/${encodeURIComponent(collectionName)}/${encodeURIComponent(String(id))}`,
        options,
      ),
      {
        method: "DELETE",
