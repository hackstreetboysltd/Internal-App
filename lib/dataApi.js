...
export async function putCollection(collectionName, data, options = {}) {
  ...
  const uid = currentUid();
  // Drop in-flight hydrates before seeding so a stale full/delta sync cannot
  // briefly restore rows that this write just removed.
  bumpSyncGeneration(collectionName, uid);
  // Always seed the local cache with the saved payload. Clearing on admin writes left
  // subscribers with [] and forced delta-sync against a dead cursor (empty upserts).
  if (Array.isArray(data)) {
    cacheManager.write(collectionName, uid, data, new Date().toISOString());
  } else {
