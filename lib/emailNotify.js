...
/** @type {string[] | null} */
let teamEmailsCache = null;
/** @type {number} */
let teamEmailsCacheAt = 0;
/** @type {unknown[] | null} */
let profileListCache = null;
/** @type {number} */
let profileListCacheAt = 0;
/** @type {string[] | null} */
let adminEmailsCache = null;
/** @type {number} */
let adminEmailsCacheAt = 0;

const EMAIL_CACHE_TTL_MS = 5 * 60 * 1000;

export function invalidateEmailNotifyCaches() {
    teamEmailsCache = null;
    teamEmailsCacheAt = 0;
    profileListCache = null;
    profileListCacheAt = 0;
    adminEmailsCache = null;
    adminEmailsCacheAt = 0;
}
...
async function getCachedProfiles() {
    const now = Date.now();
    if (profileListCache && now - profileListCacheAt < EMAIL_CACHE_TTL_MS) {
        return profileListCache;
    }
    const profiles = await get("profile", { admin: false });
    profileListCache = Array.isArray(profiles) ? profiles : [];
    profileListCacheAt = now;
    teamEmailsCache = profileListCache
        .map((p) => (p.email || "").trim().toLowerCase())
        .filter((e) => e.includes("@"));
    teamEmailsCacheAt = now;
    return profileListCache;
}
...
async function getAllTeamEmails() {
    try {
        const now = Date.now();
        if (teamEmailsCache && now - teamEmailsCacheAt < EMAIL_CACHE_TTL_MS) {
            return teamEmailsCache;
        }
        await getCachedProfiles();
        return teamEmailsCache || [];
    } catch (e) {
...
async function getAdminEmails() {
    try {
        const now = Date.now();
        if (adminEmailsCache && now - adminEmailsCacheAt < EMAIL_CACHE_TTL_MS) {
            return adminEmailsCache;
        }
        const data = await get("role_access", { admin: false });
        if (Array.isArray(data)) {
            const adminsRec = data.find((r) => r.id === "admins");
            adminEmailsCache = (adminsRec?.emails || [])
                .map((e) => String(e || "").trim().toLowerCase())
                .filter(Boolean);
        } else {
            adminEmailsCache = [];
        }
        adminEmailsCacheAt = now;
        return adminEmailsCache;
    } catch (e) {
...
async function isEmailNotificationsPaused() {
    ...
    get("settings", { admin: false }).then((data) => {
        const globalPaused = pausedFromSettings(data);
        if (globalPaused !== null) setEmailNotificationsPaused(globalPaused);
    }).catch((e) => {
        console.log("[EmailNotify] Failed to fetch settings, falling back to local storage.", e);
    });
    return emailNotificationsPaused === true;
}
