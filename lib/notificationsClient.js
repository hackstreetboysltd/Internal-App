...
import { isSecureNoticeAction } from "@/lib/secureNoticeCopy";
...
export function describeNotificationRow(row) {
  ...
  if (row.kind === "assignee") {
    if (action === "reassigned" || action === "updated") {
      return action === "reassigned"
        ? `${actor} re-assigned you a goal: ${item}`
        : `${actor} updated a goal assigned to you: ${item}`;
    }
    return `${actor} assigned you a goal: ${item}`;
  }
  if (row.kind === "direct") {
    if (isSecureNoticeAction(action)) return action;
    if (action === "approved") return `${actor} approved ${item}`;
    if (action === "rejected") return `${actor} rejected ${item}`;
    return `${actor} ${action} ${item}`.replace(/\s+/g, " ").trim();
  }
