export const MODULES = [
    ...
    { key: "channels", path: "/channels/", name: "Communication Channels", dockLabel: "Channels", dockTitle: "Communication Channels", icon: "fa-solid fa-sitemap", stubTitle: "Channels", adminOnly: true },
...
export function pathForModule(folderName, isAdmin) {
    ...
    // Channels is admin-only; leaving Admin Mode should land in Messages (membership tabs).
    if (key === "channels" && !isAdmin) return "/messages/";
...
/**
 * Dock / dashboard entries visible for the current admin view toggle.
 * @param {boolean} isAdminView
 */
export function visibleModules(isAdminView) {
    return MODULES.filter((mod) => !mod.adminOnly || isAdminView);
}
