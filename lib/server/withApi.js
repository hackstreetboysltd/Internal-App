...
import { patchSession, touchSessionThrottled } from "@/lib/server/session";
  ...
  return async function wrapped(request, routeContext) {
    ...
    try {
      ...
      if (auth || admin) {
        ...
        session = sid
          ? await touchSessionThrottled(sid, { force: !isSyncPath(path) })
          : null;
        if (!session) {
          const response = NextResponse.json({ error: "Unauthorized" }, { status: 401 });
          if (sid) clearSessionCookie(response);
