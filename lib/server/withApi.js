...
import { getRolesForEmail, reconcileSessionUser } from "@/lib/server/users";
...
export function withApi(handler, options = {}) {
  ...
  return async function wrapped(request, routeContext) {
    ...
    if (!isSyncPath(path) && !isDocumentFilePath(path) && !path.startsWith("/api/data/")) {
    ...
    const respond = async (response, status, error = null) => {
      if (!skipRequestLog(path)) {
        ...
        scheduleAfterResponse(() => logApiRequest(payload));
      }
      return response;
    };
    ...
    try {
      ...
      if (auth || admin) {
        ...
        session = await reconcileSessionUser(session, patchSession);
        if (!session) {
          const response = NextResponse.json({ error: "Unauthorized" }, { status: 401 });
          clearSessionCookie(response);
          return respond(response, 401);
