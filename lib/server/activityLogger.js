...
export function normalizeActivityBatch(events) {
  ...
  return events.map((raw, index) => {
    ...
    return {
      ...
      path,
      meta,
      clientTs: typeof raw.clientTs === "string" ? raw.clientTs : null,
    };
  });
}
...
export async function logActivityEvents(entries) {
  ...
  persistActivityLogs(entries).catch((err) => {
 ...
 * @param {ActivityLogEntry[]} entries
 */
async function persistActivityLogs(entries) {
  for (const entry of entries) {
    await query(
      `INSERT INTO activity_logs (uid, email, session_id, event_type, path, meta, ip)
       VALUES ($1::uuid, $2, $3, $4, $5, $6::jsonb, $7::inet)`,
      [
        entry.uid,
        entry.email,
        entry.sessionId,
