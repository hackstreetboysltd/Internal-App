...
import { portalTimeZone, realNowIso, realNowMs, realNowParts } from "@/lib/server/realTime";
...
function getDefaultGoalsSeed() {
  ...
  const parts = realNowParts();
  const now = new Date(parts.year, parts.month - 1, parts.day);
  const currentWeek = getWeekIdentifier(now);
  const currentMonth = `${parts.year}-M${String(parts.month).padStart(2, "0")}`;
  const currentAnnual = `${parts.year}`;
 ...
 * @param {string} nowIso
 */
async function stampCollectionWallTimes(collectionName, items, nowIso) {
  if (!STAMP_COLLECTIONS.has(collectionName)) return items;
  const previous = await listCollectionItems(collectionName);
...
export async function replaceCollectionItems(collectionName, items, authorEmail) {
  ...
  listToSave = await stampCollectionWallTimes(collectionName, listToSave, now);
  ...
  try {
    ...
    for (const item of listToSave) {
      ...
      const rowId = String(item.id ?? item.email ?? realNowMs());
      await client.query(
