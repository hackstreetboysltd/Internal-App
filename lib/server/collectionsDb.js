...
import { filterAndRedactMessagesForActor, redactMessageWrapsForActor } from "@/lib/messageRead";
import { applyMessageEditStamp } from "@/lib/messageSave";
import { sanitizeItemForClient, stripMsgIdentityEnc } from "@/lib/accountIdentity";
...
export async function applyReadInterceptors(collectionName, data, ctx = {}) {
  ...
  if (collectionName === "profile") {
    return items.map(stripMsgIdentityEnc);
  }
  return items;
}
...
function stampModuleItem(collectionName, oldItem, item, nowIso) {
  ...
  if (base === "messages") {
    Object.assign(next, applyMessageEditStamp(oldItem, next, nowIso));
    if (isNew) {
...
/**
 * @param {string} collectionName
 * @param {{ name?: string, email?: string, roles?: string[] } | null} [actor]
 *        When provided and the collection uses security levels, A-level rows
 *        the actor cannot read are omitted.
 * @param {{ adminSeesAll?: boolean }} [options]
 *        For `channels`, `adminSeesAll` lets admins list every channel (Channels module).
 */
export async function readCollection(collectionName, actor = null, options = {}) {
  assertValidCollectionName(collectionName);
  let items = await listCollectionItems(collectionName);
  items = await applyReadInterceptors(collectionName, items);
  if (actor && collectionName === "channels") {
    items = filterChannelItemsForActor(items, actor, {
      adminSeesAll: options.adminSeesAll === true,
    });
  } else if (actor && collectionName === "messages") {
    const channels = await listCollectionItems("channels");
    items = filterAndRedactMessagesForActor(items, actor, channels);
  } else if (actor && collectionUsesSecurityLevel(collectionName)) {
    items = filterItemsForActor(items, actor);
  }
  return items;
}
 ...
 * @param {string} collectionName
 */
async function listCollectionItemsOnClient(client, collectionName) {
 ...
 * @param {string[]} ids
 */
async function listCollectionItemsByIdsOnClient(client, collectionName, ids) {
  if (!ids.length) return [];
  const result = await client.query(
    `SELECT id, data FROM collection_items
     WHERE collection_name = $1 AND deleted_at IS NULL AND id = ANY($2::text[])`,
    [collectionName, ids],
  );
  return result.rows.map((row) => {
    const item = row.data && typeof row.data === "object" ? row.data : {};
 ...
 * @param {import("pg").PoolClient} client
 * @param {string} collectionName
 * @param {{ id: string, data: string, author: string | null }[]} rows
 * @param {string} now
 */
async function upsertCollectionRowsOnClient(client, collectionName, rows, now) {
 ...
 * @param {import("pg").PoolClient} client
 * @param {string} collectionName
 * @param {unknown[]} listToSave
 * @param {string | null} authorEmail
 * @param {string} now
 */
async function writeCollectionReplaceOnClient(client, collectionName, listToSave, authorEmail, now) {
  ...
  if (rows.length > 0) {
    await upsertCollectionRowsOnClient(client, collectionName, rows, now);
  }
...
export async function replaceCollectionItems(collectionName, items, authorEmail, knownItems) {
  ...
  try {
    ...
    await writeCollectionReplaceOnClient(client, collectionName, listToSave, authorEmail, now);
...
/**
 * Read-modify-write under the collection advisory lock so concurrent full-collection
 * PUTs cannot soft-delete each other's newly inserted rows (lost update).
 *
 * @param {string} collectionName
 * @param {string | null} authorEmail
 * @param {(oldItems: unknown[]) => unknown[] | Promise<unknown[]>} buildNext
 * @returns {Promise<{ oldItems: unknown[], newItems: unknown[] }>}
 */
export async function replaceCollectionItemsAtomic(collectionName, authorEmail, buildNext) {
  assertValidCollectionName(collectionName);
  if (typeof buildNext !== "function") {
    throw new Error("buildNext callback is required");
  }

  if (collectionName === "role_access") {
    const oldItems = await readRoleAccessAsCollection();
    const built = await buildNext(oldItems);
    if (!Array.isArray(built)) {
      throw new Error("Collection payload must be an array");
    }
    await writeRoleAccessCollection(built);
    return { oldItems, newItems: built };
  }
  ...
  try {
    ...
    const oldItems = await listCollectionItemsOnClient(client, collectionName);
    const built = await buildNext(oldItems);
    if (!Array.isArray(built)) {
      throw new Error("Collection payload must be an array");
    }
    ...
    const now = realNowIso();
    let listToSave = normalizeItemsForSave(collectionName, built);
    listToSave = await stampCollectionWallTimes(collectionName, listToSave, now, oldItems);
    await writeCollectionReplaceOnClient(client, collectionName, listToSave, authorEmail, now);
    await client.query("COMMIT");
    return { oldItems, newItems: listToSave };
  } catch (err) {
    try {
      await client.query("ROLLBACK");
    } catch {
...
/**
 * Insert or update only the given rows. Sibling messages are left untouched so
 * a send does not rewrite the whole collection (the path that timed out at 15s).
 *
 * @param {string} collectionName
 * @param {unknown[]} incomingItems
 * @param {string | null} authorEmail
 * @param {(
 *   oldItems: unknown[],
 *   merged: unknown[],
 * ) => unknown[] | Promise<unknown[]>} authorizeMerged
 * @returns {Promise<{ oldItems: unknown[], newItems: unknown[] }>}
 */
export async function mergeCollectionItemsAtomic(
  collectionName,
  incomingItems,
  authorEmail,
  authorizeMerged,
  ...
  if (!Array.isArray(incomingItems) || incomingItems.length === 0) {
    throw new CollectionSaveError("Merge payload must be a non-empty array", 400);
  }
  if (typeof authorizeMerged !== "function") {
    throw new Error("authorizeMerged callback is required");
  }
  if (collectionName === "role_access") {
    throw new CollectionSaveError("Merge writes are not supported for role_access", 400);
  }

  const incomingIds = new Set(
    incomingItems
      .filter((item) => item && typeof item === "object")
      .map((item) => collectionItemId(item)),
  );

  const authorizeAndStamp = async (oldItems) => {
    const byId = new Map(
      oldItems
        .filter((item) => item && typeof item === "object")
    ...
    for (const item of incomingItems) {
      if (!item || typeof item !== "object") continue;
      byId.set(String(item.id ?? collectionItemId(item)), item);
    }
    const merged = [...byId.values()];
    const authorized = await authorizeMerged(oldItems, merged);
    if (!Array.isArray(authorized)) {
      throw new Error("Collection payload must be an array");
    }
    const now = realNowIso();
    const toUpsert = authorized.filter((item) => item && incomingIds.has(String(item.id)));
    let listToSave = normalizeItemsForSave(collectionName, toUpsert);
    listToSave = await stampCollectionWallTimes(collectionName, listToSave, now, oldItems);
    const rows = dedupeItemsById(listToSave).map((row) => ({
      ...row,
      author: authorEmail || row.author,
    }));
    return { oldItems, listToSave, rows, now };
  };

  // New message sends: one INSERT, no table scan and no advisory lock. Deletes
  // still use the full PUT replace. Concurrent sends use distinct client ids.
  if (collectionName === "messages") {
    const prepared = await authorizeAndStamp([]);
    const pool = await getPool();
    await upsertCollectionRowsOnClient(pool, collectionName, prepared.rows, prepared.now);
    return { oldItems: prepared.oldItems, newItems: prepared.listToSave };
  }
  ...
  try {
    ...
    const incomingIdList = [...incomingIds];
    const oldItems = await listCollectionItemsByIdsOnClient(
      client,
      collectionName,
      incomingIdList,
    );
    ...
    return { oldItems, newItems: [...nextById.values()] };
  } catch (err) {
    try {
      await client.query("ROLLBACK");
    } catch {
...
/**
 * A `since` cursor in the future (or unparseable) must not filter the delta.
 * Firefox HAR 2026-09-10: client polled `since=2026-09-10T21:33:21.806Z` while
 * every live message `updated_at` was `2026-09-10T18:43:02.339Z` — empty
 * upserts/deletes, so peer rows that had flashed in then vanished from cache
 * never came back.
 *
 ...
 * @returns {Date | null}
 */
export function effectiveSyncSince(sinceIso, nowMs = realNowMs()) {
  if (!sinceIso) return null;
  const since = new Date(sinceIso);
  if (Number.isNaN(since.getTime())) return null;
  // 2s skew allowance for clock/NTP; anything further ahead is a bad cursor.
  if (since.getTime() > nowMs + 2000) return null;
  return since;
}
 ...
 * @param {{ adminSeesAll?: boolean }} [options]
 */
export async function getCollectionDelta(collectionName, sinceIso, actor = null, options = {}) {
  assertValidCollectionName(collectionName);
  ...
  const validSince = effectiveSyncSince(sinceIso);
  const filterSecurity = !!(actor && collectionUsesSecurityLevel(collectionName));
  const filterChannels = !!(actor && collectionName === "channels");
  const filterMessages = !!(actor && collectionName === "messages");
  const channelOpts = { adminSeesAll: options.adminSeesAll === true };
  const messageChannels = filterMessages ? await listCollectionItems("channels") : [];

  if (!validSince) {
    const items = await readCollection(collectionName, actor, channelOpts);
  ...
  for (const row of result.rows) {
    ...
    if (row.deleted_at) {
      ...
      const data = rowToCollectionItem(row.data, row.id);
      const visible = filterChannels
        ? filterChannelItemsForActor([data], actor, channelOpts).length > 0
        : filterMessages
          ? filterMessageItemsForActor([data], actor, messageChannels).length > 0
        : !filterSecurity || filterItemsForActor([data], actor).length > 0;
      if (!visible) {
        deletes.push(String(row.id));
        continue;
      }
      const payload = filterMessages
        ? (redactMessageWrapsForActor([data], actor)[0] || data)
        : sanitizeItemForClient(collectionName, data);
      upserts.push({
        id: String(row.id),
        data: payload,
