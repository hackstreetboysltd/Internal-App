...
import { scheduleAfterResponse } from "@/lib/server/afterResponse";
...
export function dispatchCollectionNotifications(params) {
  ...
  // Must not keep the serverless isolate busy — a hung EmailJS fetch used to
  // delay the collection PUT until Vercel 504'd with an empty Firefox DataApiError.
  scheduleAfterResponse(() => processCollectionNotifications({
    collectionName,
    oldItems,
    newItems,
    actor,
...
export async function processCollectionNotifications(params) {
  ...
  const profiles = collectionName === "goals" || collectionName === "pending_goals"
    ...
    : [];
  const apps = collectionName === "goals" || collectionName === "pending_goals"
    ? await listCollectionItems("apps")
    : [];

  const intents = buildCollectionIntents(collectionName, oldItems, newItems, {
    profiles,
    apps,
    actorEmail,
  });
  ...
  for (const intent of intents) {
    ...
    if (intent.kind === "assignee") {
      ...
      await sendAssigneeGoalEmail({
        ...
        previousOwnerName: intent.previousOwnerName,
        apps,
      });
      continue;
    }
    ...
    if (intent.kind === "direct") {
      ...
      if (!targetEmail || targetEmail === actorEmail) continue;
      await insertNotification({
        kind: "direct",
        module: intent.module,
        action: intent.action,
      ...
      if (intent.secureNotice === true) {
        await sendSecureNoticeEmail({
          toEmail: targetEmail,
