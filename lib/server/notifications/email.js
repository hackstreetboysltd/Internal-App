...
import { EMAILJS_SEND_URL, emailJsRequestInit } from "@/lib/server/notifications/emailFetch";
import { getEmailJsOrigin, emailPortalUrlForPath, getEmailPortalUrl } from "@/lib/server/portalPublicUrl";

function credentials() {
  const serviceId = process.env.EMAILJS_SERVICE_ID || "";
  const templateId = process.env.EMAILJS_TEMPLATE_ID || "";
  const publicKey = process.env.EMAILJS_PUBLIC_KEY || "";
  const portalUrl = getEmailPortalUrl();
  const ready = !!(
    serviceId &&
    templateId &&
    publicKey &&
...
async function sendOneEmail(toEmail, templateParams) {
  ...
  const origin = getEmailJsOrigin();
  const res = await fetch(EMAILJS_SEND_URL, emailJsRequestInit(
    origin,
    JSON.stringify({
...
export async function sendAssigneeGoalEmail(payload) {
  ...
  const catalog = Array.isArray(apps) ? apps : await listCollectionItems("apps");
...
export async function sendGoalReminderEmail(payload) {
  ...
  const apps = await listCollectionItems("apps");
 ...
 * }} payload
 */
export async function sendSecureNoticeEmail(payload) {
  const creds = credentials();
  if (!creds.ready) {
    console.warn("[Notifications] EmailJS not configured — skipping secure notice.");
    return;
  }
  ...
  const portalUrl = payload.linkPath
    ? emailPortalUrlForPath(payload.linkPath)
    : creds.portalUrl;

  const templateParams = secureNoticeEmailCopy({
  ...
  try {
    ...
    console.warn(`[Notifications] Failed to send secure notice to ${toEmail}:`, err.message);
  }
}
