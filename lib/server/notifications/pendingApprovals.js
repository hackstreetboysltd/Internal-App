...
import { linkPathForCollection, linkPathForModule } from "@/lib/server/notifications/links";
...
async function buildPendingApprovalsForAdmin() {
  ...
  for (const record of goals) {
    ...
    const type = record.type || (record.weekId ? "weekly" : "annual");
    const period = record.periodId || record.weekId || "";
    const ownerKey = email || displayName;

    for (let i = 0; i < subGoals.length; i += 1) {
      if (!goalNeedsAdminReview(subGoals[i])) continue;
      const goal = subGoals[i];
      const submittedMs = resolveGoalReviewSubmittedMs(goal, record);
      // Batch by owner + horizon + period so individual goal records (and
      // legacy multi-goal cards) accordion together in the notification panel.
      const batchKey = `goals-owner:${ownerKey}:${type}:${period}`;
      reviewCandidates.push({
        ...
        created_at: toIso(submittedMs),
        batch_key: batchKey,
        batch_label: batchLabelForGoalRecord(record),
  ...
  for (const row of reviewCandidates) {
    const key = String(row.batch_key || "");
    const isBatch = key && (batchCounts.get(key) || 0) > 1;
    rows.push({
      ...row,
      kind: "approval",
      batch_key: isBatch ? key : null,
      batch_label: isBatch ? row.batch_label : null,
    });
  }

  for (const profile of profiles) {
    if (!profile || typeof profile !== "object") continue;
    if (String(profile.approvedStatus || "").toLowerCase() !== "pending") continue;
    const email = ownerEmail(profile, profiles);
    const displayName = plainText(profile.name) || email || "New member";
    const submittedMs = msFromValue(profile.createdAt) || msFromId(profile.id);
    rows.push({
      id: `access:${email || profile.id}`,
      kind: "approval",
      module: "User Access Control",
      action: "Access request",
      item_name: email ? `${displayName} · ${email}` : displayName,
      actor_name: displayName,
      actor_email: email || null,
      actor_key: actorKey(email, displayName),
      link_path: linkPathForModule("User Access Control"),
      mandatory: true,
      read: false,
      created_at: toIso(submittedMs),
