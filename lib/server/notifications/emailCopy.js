...
export function formatGoalsDetailText(texts) {
  const clipped = texts.map((text) => text.slice(0, 200)).filter(Boolean);
...
export function buildEmailTemplateParams(fields) {
  ...
  const goalTexts = Array.isArray(fields.goalTexts)
    ? fields.goalTexts.map((text) => String(text).slice(0, 200)).filter(Boolean)
    : [];
  const detailText = String(
    fields.detailText || formatGoalsDetailText(goalTexts) || "",
  ).trim();
...
export function assigneeGoalEmailCopy(payload) {
  ...
  const subject = updated
    ? `${actorName} updated a goal assigned to you`
    : `${actorName} assigned you a goal`;
...
export function goalReminderEmailCopy(payload) {
  ...
  return buildEmailTemplateParams({
    ...
    detailText: snippet.slice(0, 200),
    note,
    timestamp: payload.timestamp,
    portalUrl: payload.portalUrl,
