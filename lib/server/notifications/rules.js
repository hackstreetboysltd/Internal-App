  ...
  for (const item of diff.created) {
    ...
    if (item.assignedByAdmin) {
      ...
      intents.push({
        ...
        module: moduleLabel,
        action: "assigned",
        itemName: formatGoalsItemName(extractGoalTexts(goalItems), `${type} goals (${periodId})`),
        mandatory: true,
 ...
 * @returns {NotificationIntent[]}
 */
function buildDefaultIntents(collectionName, diff, profiles, actorEmail = "") {
...
export function buildCollectionIntents(collectionName, oldItems, newItems, ctx = {}) {
  ...
  return buildDefaultIntents(collectionName, diff, profiles, actorEmail)
