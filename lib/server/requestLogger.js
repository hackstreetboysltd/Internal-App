...
async function persistApiRequestLog(record) {
  // Resolve uid via users table so stale Redis sessions (uid not in users)
  // still persist the log row with a NULL uid instead of FK 23503 spam.
  await query(
    `INSERT INTO api_request_logs (
      ...
      $1, $2, $3, $4::jsonb, $5, $6,
      (SELECT id FROM users WHERE id = $7::uuid), $8, $9, $10::inet, $11, $12, $13, $14::timestamptz
    )`,
    [
      record.requestId,
      record.method,
