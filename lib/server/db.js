...
import { databaseWantsSsl, normalizeDatabaseUrl } from "@/lib/server/databaseUrl.mjs";
...
async function buildPoolConfig(url) {
  const connectionString = normalizeDatabaseUrl(url) || url;
  const serverless = isServerlessRuntime();
  const ssl = databaseWantsSsl(connectionString)
    ? { rejectUnauthorized: process.env.DATABASE_SSL_REJECT_UNAUTHORIZED !== "0" }
    : undefined;

  if (isCloudDatabaseUrl(connectionString) && !serverless) {
