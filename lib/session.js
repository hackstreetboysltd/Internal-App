...
export function SessionProvider({ children, requireAuth = false }) {
    ...
    const refreshSession = useCallback(async ({ profileSync = "skip" } = {}) => {
        try {
            const data = await fetchAuthMe();
            if (!data?.user) {
                setAuthUserCache(null);
            ...
            if (profileSync === "background" && data.syncProfile) {
                syncProfileOnLogin(nextUser)
                    .then((synced) => {
                        if (!synced) return;
                        const updated = { ...nextUser, ...synced };
                        setAuthUserCache(updated);
    ...
    useEffect(() => {
        let cancelled = false;
        refreshSession({ profileSync: "background" }).finally(() => {
